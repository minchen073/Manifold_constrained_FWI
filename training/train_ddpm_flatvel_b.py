#!/usr/bin/env python3
"""
FlatVel-B DDPM training: aligned with diffusers ``examples/unconditional_image_generation/train_unconditional.py``
...
File naming: model{i}.npy (same as CurveVel-B); train i=1-48, test i=49-60.
Velocity (m/s) default linear norm ``y = (x - 3000) / 1500`` to [-1, 1] (override in YAML).

Usage:
  accelerate launch train_ddpm_flatvel_b.py --config configs/flatvel_b_ddpm.yaml
  python train_ddpm_flatvel_b.py --config configs/flatvel_b_ddpm.yaml
...
try:
    ...
    def _patch_diffusers_custom_op() -> None:
        ...
        # Patch custom_op: schema inference on torch 2.4.x fails for `float | None` annotations.
        ...
        # Patch register_fake: if the op was skipped above it won't exist, so the fake
        # registration also needs to be silenced.
...
def _apply_cuda_device_env_before_torch() -> None:
    ...
    default_cfg = training_dir / "configs" / "flatvel_b_ddpm.yaml"
...
_DEFAULT_DATA = _MANIFOLD_ROOT / "data" / "FlatVel-B"
...
class FlatVelBNpyDataset(Dataset):
    """FlatVel-B dataset: files named ``model{i}.npy``, each (N,1,70,70), default N=500.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FlatVel-B DDPM training (Accelerate)")
    parser.add_argument(
        "--config",
        type=str,
        default=str(_TRAINING_DIR / "configs" / "flatvel_b_ddpm.yaml"),
    )
    ...
    run_name = str(tcfg.get("run_name", "flatvel_b_ddpm"))
    seed = int(tcfg["seed"])
