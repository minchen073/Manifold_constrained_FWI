#!/usr/bin/env python3
"""
FlatFault-B DDPM training: same network and hyperparameters as CurveVel-B.
File naming pattern: vel{prefix}_1_{i}.npy (prefix∈{6,7,8}, train i=0-31, test i=32-35).

Usage:
  accelerate launch train_ddpm_flatfault_b.py --config configs/flatfault_b_ddpm.yaml
  python train_ddpm_flatfault_b.py --config configs/flatfault_b_ddpm.yaml

GPU: YAML ``training.cuda_device`` (default 3) sets ``CUDA_VISIBLE_DEVICES`` before ``import torch``;
...
def _apply_cuda_device_env_before_torch() -> None:
    ...
    default_cfg = training_dir / "configs" / "flatfault_b_ddpm.yaml"
...
_DEFAULT_DATA = _MANIFOLD_ROOT / "data" / "FlatFault-B"
...
class FlatFaultBNpyDataset(Dataset):
    """FlatFault-B dataset: files named ``vel{prefix}_1_{i}.npy``, each (N,1,70,70).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FlatFault-B DDPM training (Accelerate)")
    parser.add_argument(
        "--config",
        type=str,
        default=str(_TRAINING_DIR / "configs" / "flatfault_b_ddpm.yaml"),
    )
    ...
    run_name = str(tcfg.get("run_name", "flatfault_b_ddpm"))
    seed = int(tcfg["seed"])
