#!/usr/bin/env python3
"""
CurveFault-B DDPM training: same network and hyperparameters as FlatFault-B.
File naming pattern: vel{prefix}_1_{i}.npy (prefix∈{6,7,8}, train i=0-31, test i=32-35).

Usage:
  accelerate launch train_ddpm_curvefault_b.py --config configs/curvefault_b_ddpm.yaml
  python train_ddpm_curvefault_b.py --config configs/curvefault_b_ddpm.yaml

GPU: YAML ``training.cuda_device`` (default 0) sets ``CUDA_VISIBLE_DEVICES`` before ``import torch``;
...
try:
    ...
    # CurveVelB not present; apply the same fix inline.
    # diffusers 0.37 uses `float | None` annotations with torch.library.custom_op,
    # which torch 2.4.x infer_schema cannot handle. Wrap the decorator to fall back
    # to a no-op when schema inference fails.
    def _patch_diffusers_custom_op() -> None:
        import torch as _t

        _orig_custom_op = _t.library.custom_op

        def _safe_custom_op(name, fn=None, /, *, mutates_args, device_types=None, schema=None):
            if fn is not None:
                try:
                    return _orig_custom_op(name, fn, mutates_args=mutates_args, device_types=device_types, schema=schema)
                except (ValueError, RuntimeError):
                    return fn
            else:
                try:
                    decorator = _orig_custom_op(name, mutates_args=mutates_args, device_types=device_types, schema=schema)
                except (ValueError, RuntimeError):
                ...
                return _safe_decorator

        _t.library.custom_op = _safe_custom_op

        _orig_register_fake = _t.library.register_fake

        def _safe_register_fake(op, fn=None, /, *, lib=None, _stacklevel=1):
            if fn is not None:
                try:
                    return _orig_register_fake(op, fn, lib=lib, _stacklevel=_stacklevel)
                except (ValueError, RuntimeError):
                    return fn
            else:
                try:
                    decorator = _orig_register_fake(op, lib=lib, _stacklevel=_stacklevel)
                except (ValueError, RuntimeError):
                    return lambda func: func

                def _safe_fake_decorator(func):
                    try:
                        return decorator(func)
                    except (ValueError, RuntimeError):
                        return func

                return _safe_fake_decorator

        _t.library.register_fake = _safe_register_fake

    _patch_diffusers_custom_op()
    del _patch_diffusers_custom_op
...
def _apply_cuda_device_env_before_torch() -> None:
    ...
    default_cfg = training_dir / "configs" / "curvefault_b_ddpm.yaml"
...
_DEFAULT_DATA = _MANIFOLD_ROOT / "data" / "CurveFault-B"
...
def _build_openfwi_unet(
    ...
    unet_cfg_dir = _resolve_path(ucfg.get("config_dir"), _TRAINING_DIR)
    if load_pretrained:
...
class CurveFaultBNpyDataset(Dataset):
    """CurveFault-B dataset: files named ``vel{prefix}_1_{i}.npy``, each (N,1,70,70).

    ``file_prefixes`` × ``index_ids`` gives the full file list (flat index).
    ...
    def __init__(
        ...
        for prefix in file_prefixes:
            for idx in index_ids:
                p = data_root / f"vel{prefix}_1_{idx}.npy"
                if not p.is_file():
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CurveFault-B DDPM training (Accelerate)")
    parser.add_argument(
        "--config",
        type=str,
        default=str(_TRAINING_DIR / "configs" / "curvefault_b_ddpm.yaml"),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override YAML training.cuda_device.",
    )
    ...
    file_prefixes = [int(p) for p in cfg["data"]["file_prefixes"]]
    tr = cfg["data"]["train_index_range"]
    te = cfg["data"]["test_index_range"]
    ...
    sched_cfg_dir = _resolve_path(sched_block.get("config_dir"), _TRAINING_DIR)
    ...
    run_name = str(tcfg.get("run_name", "curvefault_b_ddpm"))
    seed = int(tcfg["seed"])
    ...
    train_ds = CurveFaultBNpyDataset(
        data_root,
        file_prefixes,
        train_ids,
        samples_per_file,
    ...
    if enable_xformers and is_xformers_available():
        ...
        if xv == version.parse("0.0.16"):
            print("xFormers 0.0.16 may be unstable on some GPUs; consider upgrading to >= 0.0.17.")
    ...
    if accelerator.is_main_process:
        ...
        if unet_config_yaml is not None:
            ...
            unet_meta["unet_config_dir"] = str(_resolve_path(ucfg.get("config_dir"), _TRAINING_DIR))
