...
def plot_2d(u_label: np.ndarray,
            u_predict: np.ndarray,
            file_name: str,
            title: str = "",
            save_dir: Optional[str] = None) -> None:
    r"""
    Plot the 2D images for label and prediction data.
    
    Args:
        u_label (numpy.ndarray): The label data. Shape: (32, 32)
        u_predict (numpy.ndarray): The prediction data. Shape: (32, 32)
        file_name (str): The name of the saved file.
        title (str): The title of the plot. Default: "".
        save_dir (str): The directory to save the plot. Default: None.
    ...
    # 绘制标签数据
    im1 = ax1.imshow(u_label, cmap='viridis')
    ax1.set_title('Label')
    plt.colorbar(im1, ax=ax1, fraction=0.046, pad=0.04)

    # 绘制预测数据
    im2 = ax2.imshow(u_predict, cmap='viridis')
...
def plot_l2_error_and_epochs(data: list,
                             file_name: str,
                             save_dir: Optional[str] = None, label_list = ['train', 'test'], color_list = ['b','r']) -> None:
    r""" 
    ...
    for j in range(1,len(data)):
        plt.plot(data[0], data[j], '--', color=color_list[j-1], label=label_list[j-1])
...
def plot_and_save_losses(folder_name, train_epoch_losses, train_epoch_pred_losses, train_epoch_recon_losses, train_epoch_infonce_losses,
                         test_epoch_losses, test_epoch_pred_losses, test_epoch_recon_losses, test_epoch_infonce_losses):
    plt.figure(figsize=(12, 8))

    plt.subplot(4, 1, 1)
    plt.plot(train_epoch_losses, label='Train Total Loss')
    plt.plot(test_epoch_losses, label='Test Total Loss')
    plt.yscale('log')
    plt.xlabel('Epoch')
    plt.ylabel('Total Loss')
    plt.legend()
    plt.title('Total Losses over Epochs')

    plt.subplot(4, 1, 2)
    plt.plot(train_epoch_pred_losses, label='Train Pred Loss')
    plt.plot(test_epoch_pred_losses, label='Test Pred Loss')
    plt.yscale('log')
    plt.xlabel('Epoch')
    plt.ylabel('Pred Loss')
    plt.legend()
    plt.title('Pred Losses over Epochs')

        # MSE损失
    plt.subplot(4, 1, 3)
    plt.plot(train_epoch_recon_losses, label='Train Recon Loss')
    plt.plot(test_epoch_recon_losses, label='Test Recon Loss')
    plt.yscale('log')
    plt.xlabel('Epoch')
    plt.ylabel('Recon Loss')
    plt.legend()
    plt.title('Reconstruction Losses over Epochs')

    # InfoNCE损失
    plt.subplot(4, 1, 4)
    plt.plot(train_epoch_infonce_losses, label='Train InfoNCE Loss')
    plt.plot(test_epoch_infonce_losses, label='Test InfoNCE Loss')
    plt.yscale('log')
    plt.xlabel('Epoch')
    plt.ylabel('InfoNCE Loss')
    plt.legend()
    plt.title('InfoNCE Losses over Epochs')

    plt.tight_layout()
    plt.savefig(os.path.join(folder_name, 'train_test_losses.png'))
    plt.close()

def plot_and_save_comparison(folder_name, f_reconstruct, f_predict, f, epoch, dataset_type='train', grid_size=64):
    num_samples = min(5, f_predict.size(0)) 
    for i in range(num_samples):
        vmin = min(f_predict[i].min(), f[i].min(), f_reconstruct[i].min()).item()
        vmax = max(f_predict[i].max(), f[i].max(), f_reconstruct[i].max()).item()
        plt.figure(figsize=(10, 4))        
        plt.subplot(1, 3, 1)
        im1 = plt.imshow(f_predict[i].detach().cpu().numpy().reshape(grid_size, grid_size), cmap='viridis', vmin=vmin, vmax=vmax)
        plt.title('f_predict')
        plt.colorbar(im1)

        plt.subplot(1, 3, 2)
        im2 = plt.imshow(f[i].detach().cpu().numpy().reshape(grid_size, grid_size), cmap='viridis', vmin=vmin, vmax=vmax)
        plt.title('f')
        plt.colorbar(im2)

        # 绘制 f_reconstruct
        plt.subplot(1, 3, 3)
        im3 = plt.imshow(f_reconstruct[i].detach().cpu().numpy().reshape(grid_size, grid_size), cmap='viridis', vmin=vmin, vmax=vmax)
        plt.title('f_reconstruct')
        plt.colorbar(im3)

        plt.suptitle(f'{dataset_type.capitalize()} Sample {i+1} at Epoch {epoch}')
        plt.tight_layout()
        plt.savefig(os.path.join(folder_name, f'{dataset_type}_comparison_epoch_{epoch}_sample_{i+1}.png'))
        plt.close()