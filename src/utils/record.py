...
class Record:
        ...
        if self.enable_record:
            if self.enable_summary:
                for k, v in dic.items():
                    tag = k if prefix == "" else f"{prefix}/{k}"
                    self.summary.add_scalar(tag, v, step)
                self.summary.flush()
    ...
    def close(self, table_name: str = "result.csv") -> None:
        ...
        if self.enable_record:
            if self.enable_table:
                ...
                df.to_csv(file_path)
                self.visual(self.plot_losses, df)
            if self.enable_summary:
                self.summary.close()

    def plot_losses(self, df: pd.DataFrame) -> None:
        r"""Plot training and testing losses."""
        # 绘制训练损失
        if self.model_type == ' IFNO':
            train_steps = range(len(df['train/loss']))
            test_steps = range(len(df['test/loss']))
            loss_types = ['loss', 'forward_loss', 'backward_loss', 'cond_loss', 'sol_loss']
            for loss_type in loss_types:
                plt.figure(figsize=(10, 6))
                plt.plot(train_steps, df[f'train/{loss_type}'], label=f'Train {loss_type.capitalize()}')
                plt.yscale('log')
                plt.title(f'Train {loss_type.capitalize()}')
                plt.xlabel('Epoch')
                plt.ylabel('Loss')
                plt.legend()
                plt.grid(True)
                plt.savefig(os.path.join(self.image2d_dir, f'train_{loss_type}.png'))
                plt.show()

            # 绘制测试损失
            for loss_type in loss_types:
                plt.figure(figsize=(10, 6))
                plt.plot(test_steps, df[f'test/{loss_type}'], label=f'Test {loss_type.capitalize()}')
                plt.yscale('log')
                plt.title(f'Test {loss_type.capitalize()}')
                plt.xlabel('Epoch')
                plt.ylabel('Loss')
                plt.legend()
                plt.grid(True)
                plt.savefig(os.path.join(self.image2d_dir, f'test_{loss_type}.png'))
                plt.show()

        elif self.model_type == 'Inverter':
            train_steps = range(len(df['train/loss']))
            test_steps = range(len(df['test/loss']))
            loss_types = ['loss', 'feature_loss', 'forward_loss', 'backward_loss', 'cond_loss', 'sol_loss']
            for loss_type in loss_types:
                plt.figure(figsize=(10, 6))
                plt.plot(train_steps, df[f'train/{loss_type}'], label=f'Train {loss_type.capitalize()}')
                plt.yscale('log')
    ...
    def save_ckpt(self, model: nn.Module, file_name: str = "model.ckpt") -> None:
        r"""Save model's weights to ckeckpoint file."""
        if self.enable_record:
            torch.save(model.state_dict(), os.path.join(self.ckpt_dir, file_name))
...
def init_record(rank_id: int,
                ...
                config_str: str,
                inverse_problem: bool = False,
                model_type: str = 'Inverter') -> Record:
    r'''
