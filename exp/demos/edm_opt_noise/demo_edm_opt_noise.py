...
优化标准高斯噪声 z，使 PF-ODE 采样速度与目标在归一化 [-1,1] 空间 MSE 最小。

初值约定：``latents = z * sigma_max``。

Run（Manifold_constrained_FWI 目录）:
  uv run python exp/edm_opt_noise/demo_edm_opt_noise.py \\
    --config exp/edm_opt_noise/config_edm_opt_noise.yaml
"""
...
_MANIFOLD_ROOT = Path(__file__).resolve().parents[2]  # exp/edm_opt_noise -> exp -> Manifold_constrained_FWI
...
def v_normalize(v: torch.Tensor | np.ndarray) -> torch.Tensor:
    """物理速度 (m/s) → 归一化 [-1, 1]：μ = (v − 3000) / 1500。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="EDM 初始噪声优化（图像空间 MSE）")
    parser.add_argument(
        "--config", type=str,
        default=str(_SCRIPT_DIR / "config_edm_opt_noise.yaml"),
    )
    ...
    out_dir = out_base.resolve() / f"edm_opt_noise_{ts}"
    ...
    for ax, (arr, title, cmap, vm0, vm1) in zip(axes[0], [
        ...
        (pred_vel_np, "optimized v (EDM ODE)", "viridis", vel_vmin, vel_vmax),
