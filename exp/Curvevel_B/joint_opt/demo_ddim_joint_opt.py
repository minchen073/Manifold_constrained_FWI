...
def main() -> None:
    ...
    grad_wave_norms:  list[float] = []
    grad_guide_norms: list[float] = []
    ...
    for it in range(total_steps):
        ...
        if lam > 0.0:
            ...
            grad_wave_v  = torch.autograd.grad(loss_wave,  v, retain_graph=True)[0]
            grad_guide_v = torch.autograd.grad(loss_guide, v, retain_graph=True)[0]
            grad_wave_norm_val  = grad_wave_v.norm().item()
            grad_guide_norm_val = grad_guide_v.norm().item()
            loss_v = loss_wave + lam * loss_guide
        else:
            guide_loss_val = 0.0
            grad_wave_v = torch.autograd.grad(loss_wave, v, retain_graph=True)[0]
            grad_wave_norm_val  = grad_wave_v.norm().item()
            grad_guide_norm_val = 0.0
            loss_v = loss_wave
        ...
        lambdas.append(lam)
        grad_wave_norms.append(grad_wave_norm_val)
        grad_guide_norms.append(grad_guide_norm_val)
    ...
    axes[2, 2].plot(iters, grad_wave_norms,  color="C0", label="‖∇L_wave‖")
    axes[2, 2].plot(iters, grad_guide_norms, color="C5", label="‖∇L_guide‖ (raw)", linestyle="--")
    axes[2, 2].set_title("Gradient norms w.r.t. v"); axes[2, 2].legend(fontsize=8)
    ...
    opt_z2   = _make_optimizer([z], p2_optim_type, p2_lr)
    sched_z2 = _make_scheduler(opt_z2, p2_sched_type, max(1, p2_steps), p2_eta_min)
