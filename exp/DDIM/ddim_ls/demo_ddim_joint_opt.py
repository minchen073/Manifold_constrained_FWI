...
def main() -> None:
    parser = argparse.ArgumentParser(description="Joint Alternating Optimization: v + z  (CurveVel-B)")
    parser.add_argument(
        "--config", type=str,
    ...
    num_steps = int((cfg.get("optimization") or {}).get("num_steps", 6))
    ...
    target_vel_np = np.load(model60_path)[sample_index, 0].astype(np.float32)
    ...
    print(
        f"[Joint Opt | CurveVel-B]  checkpoint={ckpt.name}  sample={sample_index}  "
    ...
    def _make_scheduler(opt, sched_type: str, t_max: int, eta_min: float):
        if sched_type == "cosine":
            return torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=t_max, eta_min=eta_min)
        return None  # "none" → constant lr

    def _make_optimizer(params, opt_type: str, lr: float):
        if opt_type == "gd":
    ...
    opt_v   = _make_optimizer([v], optim_v_type, lr_v)
    sched_v = _make_scheduler(opt_v, schedv_type, total_steps, schedv_eta_min)
    opt_z   = _make_optimizer([z], optim_z_type, lr_z)
    sched_z = _make_scheduler(opt_z, schedz_type, total_steps, schedz_eta_min)
    ...
    p2_alpha0    = float(p2_cfg.get("alpha0", 1.0))
    p2_rho       = float(p2_cfg.get("rho", 0.5))
    p2_c1        = float(p2_cfg.get("c1", 1e-4))
    p2_max_bt    = int(p2_cfg.get("max_backtracks", 30))

    print(f"\n[Phase 2 LS]  wave-domain latent opt  steps={p2_steps}  "
          f"alpha0={p2_alpha0}  rho={p2_rho}  c1={p2_c1}  max_bt={p2_max_bt}  "
    ...
    for it in range(p2_steps):
        ...
        g_sq = (g * g).sum().item()   # ||g||², descent rate = -g_sq along -g
        ...
        armijo_ok    = False
        with torch.no_grad():
            for _ in range(p2_max_bt):
                z_try     = z.detach() - alpha * g
                pred_try  = sample_custom_no_grad(
                    wrapper, alphas_cumprod, z_try, t_start, p2_nsteps, eta, final_alpha_cumprod
                )
                wave_try  = forward_wave(pred_try)
                f_new     = F.mse_loss(wave_try, target_wave).item()
                if f_new <= f0 - p2_c1 * alpha * g_sq:
                    armijo_ok = True
                    break
                alpha *= p2_rho

        # --- update z only if Armijo condition was satisfied ---
        if armijo_ok:
            with torch.no_grad():
                z.data -= alpha * g
        else:
            alpha = 0.0
            print(f"  [P2 LS] iter {it+1:4d}  WARNING: line search failed, skipping update")
        ...
        if (it + 1) % max(1, p2_steps // 5) == 0 or it == 0:
            print(
                f"  [P2 LS] iter {it+1:4d}/{p2_steps}  "
                f"wave_mse={loss_p2.item():.5f}  alpha={alpha:.2e}  "
    ...
    ax5[0, 0].set_title("target v", fontsize=8); ax5[0, 0].axis("off")
    ...
    for ax, (arr, title, cmap, vm0, vm1) in zip(ax6, [
        ...
        ax.set_title(title, fontsize=9); ax.axis("off"); plt.colorbar(im, ax=ax, fraction=0.046)
    plt.suptitle(
        f"Phase 2 final generation  |  z_phase2 → DDIM({p2_nsteps} steps)  |  model60[{sample_index}]",
        fontsize=10,
    )
    plt.tight_layout()
    plt.savefig(out_dir / "phase2_final_generation.png", dpi=200, bbox_inches="tight")
    plt.close()
    print(f"  [P2] z→DDIM({p2_nsteps}) MAE={mae_p2_final:.1f} m/s  SSIM={ssim_p2_final:.4f}")

    # --- Figure 7: Phase 2 metrics ---
    if p2_wave_losses:
        p2_iters = list(range(1, p2_steps + 1))
        fig7, ax7 = plt.subplots(1, 4, figsize=(18, 4))
        ...
        ax7[3].semilogy(p2_iters, p2_alpha_hist, color="C3")
        ax7[3].set_title("Line search step α"); ax7[3].set_xlabel("Iteration"); ax7[3].grid(True, alpha=0.3)
        plt.suptitle(f"Phase 2 metrics (line search)  |  model60[{sample_index}]", fontsize=10)
        plt.tight_layout()
    ...
    summary = {
        ...
        f"phase2_gen_{p2_nsteps}steps_mae_m_s": float(mae_p2_final),
        f"phase2_gen_{p2_nsteps}steps_ssim": float(ssim_p2_final),
