...
用 SGLD 优化初始噪声 z，使 DDIM 得到的速度经 ``seismic_master_forward_modeling``
与目标波场 MSE 最小（需 CUDA + CuPy）。

SGLD 更新规则（带温度参数）:
    z_{t+1} = z_t - lr * ∇U(z_t) + sqrt(2 * lr * T) * ε,  ε ~ N(0,I)

温度 T 的含义：
  T = 0.0  →  纯梯度下降（无噪声）
  T > 0    →  Langevin 噪声，T 越大探索越强
  启用退火：T 在迭代中从 temperature 按 anneal_schedule 衰减到 temperature_final

注意：sqrt(2*lr*T) 是绝对尺度，与梯度大小无关。需根据实际梯度量级手动设置 T。
经验做法：先以 temperature=0 跑一次观察 ‖∇z‖ 的典型值，再令 sqrt(2*lr*T) 约为
lr*‖∇z‖ 的某个比例（例如 10%），即 T ≈ 0.05 * (‖∇z‖ / sqrt(d))^2 / lr。

初值约定：z ~ N(0, I)，shape (1,1,70,70)，直接作为 DDIM 的 x_T。

Run（Manifold_constrained_FWI 目录）:
  uv run python exp/DDIM/ddim_sgld_wave/demo_ddim_sgld_wave.py \\
    --config exp/DDIM/ddim_sgld_wave/config_ddim_sgld_wave.yaml
"""
...
_MANIFOLD_ROOT = Path(__file__).resolve().parents[3]  # exp/DDIM/ddim_sgld_wave -> DDIM -> exp -> Manifold_constrained_FWI
...
def get_temperature(it: int, opt_steps: int, T0: float, T1: float, schedule: str) -> float:
    """计算第 it 步（0-indexed）的当前温度。

    若 T1 >= T0（无退火），直接返回 T0。
    schedule: "cosine" 或 "linear"。
    """
    if T1 >= T0 or opt_steps <= 1:
        return T0
    progress = it / (opt_steps - 1)  # [0, 1]
    if schedule == "cosine":
        factor = 0.5 * (1.0 + math.cos(math.pi * progress))
        return T1 + (T0 - T1) * factor
    else:  # linear
        return T0 + (T1 - T0) * progress


def sgld_step(z: torch.Tensor, lr: float, temperature: float) -> torch.Tensor:
    """原地执行 SGLD 一步，返回注入的噪声 tensor（用于记录）。

    noise_std = sqrt(2 * lr * T)
    z_{t+1} = z_t - lr * ∇U(z_t) + noise_std * ε

    temperature=0 时退化为纯梯度下降。
    """
    assert z.grad is not None, "sgld_step called before backward"
    with torch.no_grad():
        grad = z.grad.clone()
        if temperature > 0.0:
            noise_std = math.sqrt(2.0 * lr * temperature)
            noise = torch.randn_like(z) * noise_std
        else:
            noise = torch.zeros_like(z)
        z.data.add_(-lr * grad + noise)
        z.grad = None
    return noise


def main() -> None:
    parser = argparse.ArgumentParser(description="DDIM + SGLD 初始噪声优化（波场 MSE）")
    parser.add_argument(
        "--config", type=str,
        default=str(_SCRIPT_DIR / "config_ddim_sgld_wave.yaml"),
    )
    ...
    # SGLD 温度参数
    temperature = float(opt_cfg.get("temperature", 0.0))
    temperature_final_raw = opt_cfg.get("temperature_final", None)
    temperature_final = float(temperature_final_raw) if temperature_final_raw is not None else temperature
    anneal_schedule = str(opt_cfg.get("anneal_schedule", "cosine")).lower()
    use_annealing = temperature_final < temperature
    ...
    out_dir = out_base.resolve() / f"ddim_sgld_wave_{ts}"
    ...
    def capture(
        label: str,
        pred_n_grad_np: np.ndarray | None = None,
        grad_z_np: np.ndarray | None = None,
    ) -> None:
        ...
        # grad_z_np 由调用方在 sgld_step 清除 z.grad 之前显式传入；
        # iter 0 初始快照（尚未 backward）时退回零矩阵。
        if grad_z_np is not None:
            grad_np = grad_z_np
        elif z.grad is not None:
            grad_np = z.grad.detach().cpu().numpy().squeeze().copy()
        else:
            grad_np = np.zeros(z.shape[-2:], dtype=np.float32)
    ...
    anneal_str = (
        f"  anneal={anneal_schedule} T:{temperature}→{temperature_final}"
        if use_annealing else f"  T={temperature} (fixed)"
    )
    print(
        f"[DDIM sgld_wave] checkpoint={ckpt.name}  steps={num_steps}  eta={eta}  "
        f"opt_steps={opt_steps}  lr={lr}  reg_weight={reg_weight}"
        f"{anneal_str}  sample_index={sample_index}  seed_z={seed_z}  device={device}"
    )

    for it in range(opt_steps):
        T_cur = get_temperature(it, opt_steps, temperature, temperature_final, anneal_schedule)
        temperature_hist.append(T_cur)
        ...
        # 在 sgld_step 清除 z.grad 之前，保存本次梯度供快照使用
        saved_grad_z_np = z.grad.detach().cpu().numpy().squeeze().copy() if z.grad is not None else None

        # --- SGLD 更新 ---
        noise = sgld_step(z, lr, T_cur)
        ...
        noise_norm_hist.append(noise_norm)

        with torch.no_grad():
            pv = v_denormalize_np(sample_no_grad(wrapper, ddim_sched, z.detach(), num_steps, eta))
        ...
        if (it + 1) in snap_times:
            ...
            capture(f"iter {it + 1}", pred_n_grad_np=gv, grad_z_np=saved_grad_z_np)

        if (it + 1) % max(1, opt_steps // 8) == 0 or it == 0:
            noise_std_cur = math.sqrt(2.0 * lr * T_cur) if T_cur > 0 else 0.0
            if reg_weight > 0.0:
                print(
                    f"iter {it+1}/{opt_steps}  T={T_cur:.4e}  noise_std={noise_std_cur:.4e}  "
                    f"total={loss.item():.6f}  wave={wave_mse.item():.6f}  reg={reg_val:.6f}  "
                    f"MAE={mae_v:.1f} m/s  SSIM={ssim_v:.4f}  "
                    f"||∇z||={grad_total:.4e}  ||noise||={noise_norm:.4e}"
                )
            else:
                print(
                    f"iter {it+1}/{opt_steps}  T={T_cur:.4e}  noise_std={noise_std_cur:.4e}  "
                    f"wave MSE={loss.item():.6f}  "
                    f"MAE={mae_v:.1f} m/s  SSIM={ssim_v:.4f}  "
                    f"||∇z||={grad_total:.4e}  ||noise||={noise_norm:.4e}"
                )
    ...
    all_z_vals = np.concatenate([z_init_np.ravel()] + [hz.ravel() for hz in hist_z])
    z0lim = max(3.0, float(np.percentile(np.abs(all_z_vals), 99)) + 0.1)
    ...
    plt.suptitle(
        f"Wave MSE | DDIM+SGLD steps={num_steps} eta={eta} "
        f"T={temperature}→{temperature_final} | "
    ...
    for ax, (arr, title, cmap, vm0, vm1) in zip(axes[0], [
        ...
        (pred_vel_np, "pred v (m/s) [DDIM+SGLD]", "viridis", vel_vmin, vel_vmax),
    ...
    if use_reg:
        ...
        axes[3, 0].plot(iters, noise_norm_hist, color="C4")
        axes[3, 0].set_ylabel("||noise|| (Langevin)")
        axes[3, 0].set_xlabel("Iteration")
        axes[3, 0].set_title("Injected noise norm")
        axes[3, 0].grid(True, alpha=0.3)

        axes[3, 1].plot(iters, temperature_hist, color="C6")
        axes[3, 1].set_ylabel("noise_scale")
        axes[3, 1].set_xlabel("Iteration")
        axes[3, 1].set_title("SGLD noise_scale schedule")
        ...
        axes[2, 0].plot(iters, noise_norm_hist, color="C4")
        axes[2, 0].set_ylabel("||noise|| (Langevin)")
        axes[2, 0].set_xlabel("Iteration")
        axes[2, 0].set_title("Injected noise norm")
        axes[2, 0].grid(True, alpha=0.3)

        axes[2, 1].plot(iters, temperature_hist, color="C6")
        axes[2, 1].set_ylabel("noise_scale")
        axes[2, 1].set_xlabel("Iteration")
        axes[2, 1].set_title("SGLD noise_scale schedule")
        axes[2, 1].grid(True, alpha=0.3)

    plt.suptitle("Optimization metrics (DDIM + SGLD)", fontsize=10)
    plt.tight_layout()
    ...
    summary = {
        ...
        "anneal_schedule": anneal_schedule if use_annealing else "none",
        "lr": lr,
