...
Two-variable alternating scheme:
  ...
  z  (latent code, manifold prior)
     - Init: z ~ N(0, I)
     - Loss: MSE(DDIM(z), v.detach())   [like demo_ddim_opt_noise, target = current v]
...
Per outer iteration:
  ...
  3. Update v for 1 step                  (wave loss + λ·RED-diff guidance from v_gen)

Run (from Manifold_constrained_FWI/):
  uv run python exp/DDIM/ddim_joint_opt/demo_ddim_joint_opt.py \\
    --config exp/DDIM/ddim_joint_opt/config_ddim_joint_opt.yaml
"""
...
def load_model(checkpoint: Path, training_yaml: Path | None):
    ...
    # clip_sample=False: keep gradient signal through clamping boundary
...
def sample_with_grad(
    ...
    """DDIM forward with full gradient graph kept (for z optimization).

    z: (1,1,70,70) → returns (1,1,70,70) in [-1,1], gradients flow back to z.
    """
...
def sample_no_grad(
    ...
    """DDIM forward without gradient (for visualization and v guidance)."""
    with torch.no_grad():
...
#
# Standard set_timesteps(N) starts at t = (N-1)/N * T (e.g. N=3 → t=666).
# That t_start is baked into z after optimization.  These functions keep
# t_start fixed regardless of num_steps, so a Phase-2 sampler with more
# steps discretises the *same* ODE trajectory more finely.
#
# For num_steps=3, t_start=666 the custom sampler produces [666, 333, 0]
# — identical to the standard 3-step DDIM — so there is no behaviour change
# when num_steps matches Phase 1.
# ---------------------------------------------------------------------------

def build_ddim_timesteps(t_start: int, num_steps: int) -> list[int]:
    """Linearly spaced timesteps from t_start down to 0 (num_steps values).

    Examples
    --------
    t_start=666, num_steps=3  → [666, 333, 0]
    t_start=666, num_steps=20 → [666, 631, 596, ..., 35, 0]
    """
    return [
...
def _ddim_step(
    ...
    """Single DDIM update x_t → x_{t_prev} using explicit alpha look-up.

    prediction_type='epsilon', clip_sample=False (gradients flow freely).
    t_prev < 0 triggers use of final_alpha_cumprod (= 1.0 by default).
    """
    ...
    # ε-prediction → x_0 estimate
    pred_x0  = (sample - beta_t.sqrt() * model_output) / acp_t.sqrt()

    # DDIM variance  σ²_t = η² · (1-ᾱ_{t-1})/(1-ᾱ_t) · (1 - ᾱ_t/ᾱ_{t-1})
...
def sample_custom_with_grad(
    ...
    """DDIM from t_start → 0 in num_steps steps; gradients flow back to z."""
...
def sample_custom_no_grad(
    ...
    """DDIM from t_start → 0 in num_steps steps; no gradient."""
    with torch.no_grad():
...
def forward_wave(v: torch.Tensor) -> torch.Tensor:
    """v in [-1,1], shape (1,1,70,70) → wave (5,1000,70), gradient flows."""
    v_phys = v_denormalize_tensor(v.squeeze()).clamp(1500.0, 4500.0)
    return seismic_master_forward_modeling(v_phys)


# ---------------------------------------------------------------------------
# guidance: direct L2 toward v_gen
# ---------------------------------------------------------------------------

def guidance_from_vgen(v: torch.Tensor, v_gen: torch.Tensor) -> torch.Tensor:
    """Direct L2 guidance: pull v toward the manifold velocity v_gen.

    L_guide = ||v − v_gen.detach()||²

    Gradient ∂L_guide/∂v = 2·(v − v_gen)/N, pulling v toward v_gen.

    Args:
        v:     current auxiliary velocity (1,1,70,70), requires_grad=True
        v_gen: DDIM-decoded velocity from z (1,1,70,70), detached
...
def build_v_init(target_vel_np: np.ndarray, smooth_sigma: float, device: torch.device) -> torch.Tensor:
    """Gaussian-smoothed initial velocity model in [-1,1] space."""
...
def compute_lambda(step: int, warmup_steps: int, ramp_steps: int, lambda_max: float) -> float:
    """
    λ = 0                          for step < warmup_steps
    λ = λ_max · progress           for warmup_steps ≤ step < warmup_steps + ramp_steps
    λ = λ_max                      for step ≥ warmup_steps + ramp_steps

    where progress = (step - warmup_steps) / ramp_steps ∈ [0, 1]
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Joint Alternating Optimization: v + z")
    parser.add_argument(
        "--config", type=str,
    ...
    num_steps       = int(dcfg.get("num_steps", 20))   # Phase 1 DDIM steps (determines t_start)
    num_steps_final = int(dcfg.get("num_steps_final", 20))
    eta             = float(dcfg.get("eta", 0.0))

    # t_start: the effective starting timestep for z — derived from Phase 1 num_steps.
    # Phase 2 and final generation use the custom sampler starting from the same t_start,
    # so more steps just refine the same ODE trajectory without changing what z encodes.
    ddim_sch.set_timesteps(num_steps)
    t_start             = int(ddim_sch.timesteps[0].item())
    alphas_cumprod      = ddpm_sch.alphas_cumprod          # (T,) float32 on CPU
    ...
    raw           = np.load(model60_path)[sample_index, 0].astype(np.float32)
    target_vel_np = raw
    ...
    v_init_np = v.detach().cpu().numpy().squeeze().copy()
    ...
    print(
        f"[Joint Opt]  checkpoint={ckpt.name}  sample={sample_index}  "
    ...
    for it in range(total_steps):
        ...
        # ── Step 1: Update z (target = current v) ────────────────────────────
        loss_z_val = 0.0
        for _ in range(z_steps_per_iter):
            opt_z.zero_grad(set_to_none=True)
            v_gen_z = sample_with_grad(wrapper, ddim_sch, z, num_steps, eta)  # (1,1,70,70)
        ...
        # ── Step 2: Decode v_gen from updated z (no_grad, for v guidance) ───
        v_gen = sample_no_grad(wrapper, ddim_sch, z, num_steps, eta)  # (1,1,70,70)

        # ── Step 3: Update v (FWI + RED-diff guidance from v_gen) ────────────
        ...
        # guidance from v_gen (direct L2 pull toward manifold velocity)
        if lam > 0.0:
        ...
        if (it + 1) % max(1, total_steps // 10) == 0 or it == 0:
            print(
                ...
                f"wave={loss_wave.item():.5f}  "
                f"guide={guide_loss_val:.5f}  "
                f"z_mse={loss_z_val:.5f}  "
                f"λ={lam:.3f}  "
    ...
    # --- Figure 1: evolution snapshots (4 rows: v, v_gen, v-v_gen, z) ---
    ...
    # col 0: reference column — row 0 shows target, rows 1-3 show row labels
    axes[0, 0].imshow(target_vel_np, cmap="viridis", aspect="auto", vmin=vel_vmin, vmax=vel_vmax)
    axes[0, 0].set_title("target v", fontsize=8)
    axes[0, 0].axis("off")
    ...
    # row title for row 0 on the left
    axes[0, 0].set_ylabel(row_titles[0], fontsize=8)

    # z colorscale: symmetric around 0, at least ±3
    ...
    for j, (vv, vg, zj, lbl, mv, sv, mz, sz) in enumerate(zip(
        ...
        # row 0: v (physical) — title: iter label + v metrics
        ...
        # row 1: v_gen (manifold) — title: vgen metrics
        axes[1, col].imshow(vg, cmap="viridis", aspect="auto", vmin=vel_vmin, vmax=vel_vmax)
        axes[1, col].set_title(
            f"MAE={mz:.0f} m/s  SSIM={sz:.3f}", fontsize=7
        )
        axes[1, col].axis("off")

        # row 2: v − v_gen difference
        diff = vv - vg
    ...
    plt.suptitle(
        f"Joint Opt evolution  |  warmup={warmup_steps}  λ_max={lambda_max}  model60[{sample_index}]",
        fontsize=9,
    )
    plt.tight_layout()
    ...
    plt.suptitle(
        f"Joint Opt  |  warmup={warmup_steps}  λ_max={lambda_max}  model60[{sample_index}]\n"
    ...
    plt.suptitle(
        f"Joint Opt metrics  |  warmup={warmup_steps}  ramp={ramp_steps}  λ_max={lambda_max}  "
    ...
    # --- Figure 4: final generation from optimized z (custom sampler, same t_start) ---
    ...
    plt.suptitle(
        f"Final generation  |  z_opt → DDIM({GEN_STEPS} steps)  |  model60[{sample_index}]",
        fontsize=10,
    )
    plt.tight_layout()
    ...
    # =========================================================================
    # Phase 2: wave-domain latent optimization (warm-started from Phase 1 z)
    # =========================================================================
    ...
    for it in range(p2_steps):
        ...
        if (it + 1) % max(1, p2_steps // 5) == 0 or it == 0:
            print(
                ...
                f"wave_mse={loss_p2.item():.5f}  "
                f"MAE={mae_p2:.1f}  SSIM={ssim_p2:.4f}"
            )

    # --- Figure 5: Phase 2 evolution snapshots ---
    ...
    ax5[0, 0].set_title("target v", fontsize=8)
    ax5[0, 0].axis("off")
    for row_idx in range(1, p2_nrows):
        ax5[row_idx, 0].text(
            0.5, 0.5, p2_row_titles[row_idx],
            transform=ax5[row_idx, 0].transAxes,
            fontsize=8, va="center", ha="center",
            bbox=dict(boxstyle="round,pad=0.4", facecolor="#eeeeee", edgecolor="gray", alpha=0.9),
        )
    ...
    plt.suptitle(
        f"Phase 2 evolution  |  wave-domain latent opt  |  model60[{sample_index}]",
        fontsize=9,
    )
    plt.tight_layout()
    ...
    # --- Figure 6: Phase 2 final generation (custom sampler, same t_start) ---
    with torch.no_grad():
    ...
    mae_p2_final, ssim_p2_final = velocity_mae_ssim(
        v_p2_final_np, target_vel_np, device, vel_vmin, vel_vmax
    )
    ...
    for ax, (arr, title, cmap, vm0, vm1) in zip(ax6, [
        ...
        plt.colorbar(im, ax=ax, fraction=0.046)
    plt.suptitle(
        f"Phase 2 final generation  |  z_phase2 → DDIM({num_steps_final} steps)  |  model60[{sample_index}]",
        fontsize=10,
    )
    plt.tight_layout()
    ...
    if p2_wave_losses:
        ...
        plt.suptitle(f"Phase 2 metrics  |  model60[{sample_index}]", fontsize=10)
        plt.tight_layout()
